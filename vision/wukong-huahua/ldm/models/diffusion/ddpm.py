...
class DDPM(nn.Cell):
    ...
    def __init__(self,
        ...
        if self.use_ema:
            ...
            print(
                f"Keeping EMAs of {len(self.model_ema.untrainable_params())}.")
        ...
        self.logvar = ms.numpy.full(
            shape=(self.num_timesteps,), fill_value=logvar_init)
    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear", timesteps=1000,
        ...
        self.sqrt_one_minus_alphas_cumprod = to_mindspore(
            np.sqrt(1. - alphas_cumprod))
        self.log_one_minus_alphas_cumprod = to_mindspore(
            np.log(1. - alphas_cumprod))
        self.sqrt_recip_alphas_cumprod = to_mindspore(
            np.sqrt(1. / alphas_cumprod))
        self.sqrt_recipm1_alphas_cumprod = to_mindspore(
        ...
        self.posterior_log_variance_clipped = to_mindspore(
        ...
        if self.parameterization == "eps":
            ...
            lvlb_weights = 0.5 * \
                ms.numpy.sqrt(ms.Tensor(alphas_cumprod)) / \
                (2. * 1 - ms.Tensor(alphas_cumprod))
        else:
    ...
    def ema_scope(self, context=None):
        if self.use_ema:
            ...
            trained_parameters = [
                param for param in parameters if param.requires_grad is True]
